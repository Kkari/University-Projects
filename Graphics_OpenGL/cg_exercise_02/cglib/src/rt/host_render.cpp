		...
		|| context.params.change_requires_restart(oldParams))
		{
			...
			if (oldParams.scene != context.params.scene) {
				...
				switch (context.params.scene) {
					case RaytracingParameters::CORNELL_BOX: context.scene = context.scenes["cornell_box"]; break;
					case RaytracingParameters::SPHERE_PORTRAIT: context.scene = context.scenes["sphere_portrait"]; break;
				default:
