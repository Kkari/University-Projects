				...
				)
{
	...
	float lowThresh = 0.5;
	float highThresh = 0.9;

    const float EDGE = 1;

    float magnitude = d_Src[gid_y * Pitch + gid_x];
    
    if (magnitude >= highThresh)
        d_Dst[gid_y * Pitch + gid_x] = EDGE;
...
__kernel void CompleteHysteresis(
				__global float* d_Dst,
				__global const float* d_Src,
                uint Width,  // Use width to check for image bounds
				uint Height,
				...
				)
{
  __local float tile[TILE_Y + 2][TILE_X + 2];
	...
	float highThresh = 0.9;
	const float EDGE = 1;
	__local uint found_edge_piece;
    float magnitude = tile[lid_y + 1][lid_x + 1];
    found_edge_piece = 1;

	if (magnitude >= highThresh)
		tile[lid_y + 1][lid_x + 1] = EDGE;
	else if (magnitude <= lowThresh)
	    tile[lid_y + 1][lid_x + 1] = 0;

	while (found_edge_piece) 
	{
        barrier(CLK_LOCAL_MEM_FENCE);
        found_edge_piece = 0;
        barrier(CLK_LOCAL_MEM_FENCE);
                ...
                ) 
        {
			for (int i = -KERNEL_WIDTH; i <= KERNEL_WIDTH; i++)
			{
				for (int j = -KERNEL_WIDTH; j <= KERNEL_WIDTH; j++)
				{
                    ...
                    //printf("accessing: x: %d, y: %d\n", (int)lid_x + j, (int)lid_y + i);
					if(tile[(int)lid_y + i][(int)lid_x + j] == EDGE)
					{
						tile[lid_y][lid_x] = EDGE;
						found_edge_piece = 1;
					}
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    d_Dst[gid_y * Pitch + gid_x] = tile[lid_y + 1][lid_x + 1];
}
