...
///////////////////////////////////////////////////////////////////////////////
  __kernel void Integrate(unsigned int width,
    ...
    if(particleID > width - 1 || (particleID & (7)) != 0)
    {
        ...
        float4 pos = d_pos[GID.y * width + GID.x];
        float4 pos_prev = d_prevPos[GID.y * width + GID.x];
        ...
        // we have to account for the first moment
		...
		// Write it back.
                d_pos[GID.y * width + GID.x] = pos_next;
                d_prevPos[GID.y * width + GID.x] = pos;
...
__kernel void SatisfyConstraints(unsigned int width,
    ...
    // Allocating the local memory for the Halo, we need a halo with a width of 2, because of
    // the bend constraining elements.
    ...
    // Load up the tile, so we can work in the local memory with the constraints
    //
    // This row loads the main elements into the tile
	tile[LID.x + 2][LID.y + 2] = d_posIn[GID.x + GID.y * width];

    // From here on we load the Halo in
	//left columns
	if (LID.x < 2)
		if (GID.x - 2 >= 0)
			tile[LID.x + 2 - 2][LID.y + 2] = d_posIn[GID.x - 2 + GID.y * width];

	//top rows
	if (LID.y < 2)
		if (GID.y - 2 >= 0)
			tile[LID.x + 2][(LID.y + 2 - 2)] = d_posIn[GID.x + (GID.y - 2)*width];

	//right columns
    // We offset by 3, because we index from 0
	if (LID.x > LS.x - 3)
		if (GID.x + 2 < width)
			tile[LID.x + 2 + 2][(LID.y + 2)] = d_posIn[GID.x + 2 + GID.y*width];
	...
	if (LID.x < 2 && LID.y < 2)
		if (GID.x - 2 >= 0 && GID.y - 2 >= 0)
			tile[LID.x + 2 - 2][(LID.y + 2 - 2)] = d_posIn[GID.x - 2 + (GID.y - 2)*width];
	...
	if (particleID > width - 1 || (particleID & (7)) != 0)
	{
		// Structural, it preserves the original adjacency
        // These are the four orthogonal neighbours of the particle.
		if (GID.y > 0)
		{
			res += SatisfyConstraint(
                    tile[LID.x + 2][(LID.y + 2)], 
                    tile[LID.x + 2][(LID.y + 2 - 1)], 
                    restDistance) * WEIGHT_ORTHO;
		}
		...
		// Shear, it supports the grid like structure
        // These are the four diagonal neighbours of the particle
		if (GID.y > 0 && GID.x > 0)
		{
			res += SatisfyConstraint(
                    tile[LID.x + 2][(LID.y + 2)], 
                    tile[LID.x + 2 - 1][(LID.y + 2 - 1)], 
                    restDistance*ROOT_OF_2) * WEIGHT_DIAG;
		}
		...
		// Bend ORTHO, it affects the stiffnes of the cloth
        // These are orthogonal neighbours again, but offsetted by 2.
		if (GID.y > 1)
		{
			res += SatisfyConstraint(
                    tile[LID.x + 2][(LID.y + 2)], 
                    tile[LID.x + 2][(LID.y + 2 - 2)],
                    restDistance * 2) * WEIGHT_ORTHO_2;
		}
		...
		// Bend DIAG
        // These are the neighbours 2 diagonal elements away,
        // they are also accounting for the stiffness
		if (GID.y > 1 && GID.x > 1)
		{
			res += SatisfyConstraint(
                    tile[LID.x + 2][(LID.y + 2)],
                    tile[LID.x + 2 - 2][(LID.y + 2 - 2)],
                    restDistance*DOUBLE_ROOT_OF_2) * WEIGHT_DIAG_2;
		}
...
__kernel void CheckCollisions(unsigned int width,
    ...
    // If there is no particle for this thread, then return.
	if (get_global_id(0) >= width || get_global_id(1) >= height)
		return;

    // Get the distance between the center of the sphere and the particle
    // When we take the dot product of a vector with itself we get the square
    // of it's length.
    float4 diff = d_pos[GID.y*width + GID.x] - spherePos;
    float distance = dot3(diff, diff);

    // Because squaering is monotonic, we can compare the squared distance
    // with the square of the sphere radius.
	if (distance < sphereRad*sphereRad)
	{
        // Here we move the cloth up, onto the surface of the sphere.
