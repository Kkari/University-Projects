...
glm::vec3 evaluate_phong_BRDF(
	RenderData &data,			// class containing raytracing information
	MaterialSample const& mat,	// the material at position
	glm::vec3 const& L,			// world space direction to light (already normalized)
	glm::vec3 const& N,			// normal at the position (already normalized)
	glm::vec3 const& V)			// view vector (already normalized)
{
	cg_assert(std::fabs(glm::length(L) - 1.f) < EPSILON);
	cg_assert(std::fabs(glm::length(N) - 1.f) < EPSILON);
	cg_assert(std::fabs(glm::length(V) - 1.f) < EPSILON);

	glm::vec3 diffuse(0.f);
	if (data.context.params.diffuse) {
		diffuse = std::max(0.f, glm::dot(N, L)) * mat.k_d / float(M_PI) ;
	}
	...
	if (data.context.params.specular) {
		if (glm::dot(L, N) > 0.f) {
			const glm::vec3 R = reflect(L, N);
			specular = std::pow(std::max(0.f, glm::dot(R, V)), mat.n) * (mat.n + 1.0f) / (2.0f * float(M_PI)) * mat.k_s;
		}
	}
...
glm::vec3 trace_recursive(RenderData & data, Ray const& ray, int depth)
{
	...
	if (data.context.params.ao) {
		const float ao = evaluate_ambient_occlusion(data, isect.position, N);
		return glm::vec3(ao);
	}

    if (!hit_backside) {
		bool& distributed_recursion = data.tld->distributed_recursion;
		if (!distributed_recursion) {
			struct DistributedRecursionGuard { bool& dr; DistributedRecursionGuard(bool& dr_) : dr(dr_) { dr = true; } ~DistributedRecursionGuard() { dr = false; } } g(distributed_recursion);
			if (data.context.params.disable_direct) {
				mat.k_d = glm::vec3(0.0f);
				mat.k_s = glm::vec3(0.0f);
			}
			contribution += evaluate_illumination(data, mat, isect.position, N, V, depth);
		}
		if (distributed_recursion)
			contribution += evaluate_illumination(data, mat, isect.position, N, V, depth);
    }
